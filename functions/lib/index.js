            ...
            for (const habitDoc of habitsSnapshot.docs) {
                ...
                if (reminderTime === currentTime) {
                    ...
                    try {
                        ...
                        const response = await messaging.sendEachForMulticast(multicastMessage, pushTokens);
                        console.log(`Successfully sent ${response.successCount} notifications to user ${userId} for habit ${habit.name}`);
                        ...
                        if (response.failureCount > 0) {
                            ...
                            response.responses.forEach((resp, idx) => {
                                if (!resp.success) {
                                    ...
                                    const errorCode = err && 'code' in err && err.code
                                        ? err.code
                                        : 'unknown';
                        ...
                        remindersWithError++;
                        console.error(`Error sending reminder for habit ${habit.name} to user ${userId}:`, error);
                    }
                }
            }
        }
        console.log(`Job completed. Processed: ${remindersProcessed}, Sent: ${remindersSent}, Errors: ${remindersWithError}`);
        return null;
    }
    catch (error) {
